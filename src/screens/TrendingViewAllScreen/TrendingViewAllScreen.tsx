...
interface TrendingViewAllScreenProps {
  onOpenMusicPlayer?: (song: Song) => void;
}

export const TrendingViewAllScreen = ({ onOpenMusicPlayer }: TrendingViewAllScreenProps): JSX.Element => {
  ...
  const fetchTopTenSongs = async () => {
    ...
    try {
      // Fetch manual trending songs first
      const manualSongs = await getManualTrendingSongs('global_trending');
      ...
      // Fetch auto-trending songs
      const { data, error: fetchError } = await supabase.rpc('get_shuffled_trending_songs', {
        days_param: null,
        limit_param: 50
      });
      ...
      let filteredManualSongs = formattedManualSongs;
      ...
      if (selectedGenre !== 'all') {
        ...
        if (songGenres && songGenres.length > 0) {
          ...
          // Filter both auto-trending and manual songs by genre
          filteredData = filteredData.filter((song: any) => genreSongIds.has(song.id));
          filteredManualSongs = formattedManualSongs.filter(song => genreSongIds.has(song.id));
        } else {
          // No songs in this genre at all
          setTopTenSongs([]);
      ...
      // Combine: filtered manual songs first (ordered by display_order), then auto-trending
      // Remove duplicates (manual songs take priority)
      const manualSongIds = new Set(filteredManualSongs.map(s => s.id));
      const autoSongsFiltered = formattedAutoSongs.filter(s => !manualSongIds.has(s.id));
      const combinedSongs = [...filteredManualSongs, ...autoSongsFiltered];
      ...
      // Fallback to auto-trending only if manual fetch fails
      try {
        const { data, error: fetchError } = await supabase.rpc('get_shuffled_trending_songs', {
          days_param: null,
          limit_param: 10
  ...
  const fetchAdditionalSongs = async () => {
    ...
    try {
      ...
      // Combine: filtered manual songs first, then auto-trending
      // Remove duplicates (manual songs take priority)
      const manualSongIds = new Set(filteredManualSongs.map(s => s.id));
      const autoSongsFiltered = formattedAutoSongs.filter(s => !manualSongIds.has(s.id));
      const combinedSongs = [...filteredManualSongs, ...autoSongsFiltered];

      // Get songs starting from position 11 (index 10) onwards
      const additionalSongsList = combinedSongs.slice(10);
      ...
      try {
        ...
        let filteredData = (data || []).slice(10);
        ...
        // Get songs starting from position 11 (index 10) onwards
        const additionalSongsList = formattedSongs.slice(10);
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-[#1a1a1a] via-[#0d0d0d] to-[#000000] text-white content-with-nav overflow-y-auto">
      ...
      {isLoadingTopTen ? (
        ...
        <>
          ...
          {/* Additional Songs Grid (11-30) */}
          <div className="px-5 py-6">
            <h2 className="text-xl font-bold mb-4">More Trending {getSelectedGenreName()} Songs</h2>
            {isLoadingAdditional ? (
              <div className="grid grid-cols-3 gap-3">
