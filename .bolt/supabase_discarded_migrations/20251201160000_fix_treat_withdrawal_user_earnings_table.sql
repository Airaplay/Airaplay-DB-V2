/*
  ...
  ## Problem
  The `process_treat_withdrawal` function is trying to INSERT into a non-existent `user_earnings` table,
  causing the error: "relation 'User_earnings' does not exist"

  ## Solution
  Replace the INSERT INTO user_earnings with UPDATE users.total_earnings, which is the correct approach
  for adding funds to a user's Live Balance.

  ## Changes
  - Remove INSERT INTO user_earnings statement
  - Add UPDATE users.total_earnings statement
  - Add user record locking for consistency
  - Update transaction metadata to include live balance tracking
*/
