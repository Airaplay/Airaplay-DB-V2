/*
  # Allow Creator Pool Distribution from SQL Editor

  Problem:
  - Supabase SQL Editor typically executes without a user JWT, so auth.uid() is NULL.
  - admin_distribute_creator_pool_for_date() checks users.role via auth.uid() and fails in SQL Editor.

  Fix:
  - Permit execution when auth.uid() IS NULL (SQL Editor / service context).
  - Keep strict admin-only enforcement for authenticated calls from the app.
*/
