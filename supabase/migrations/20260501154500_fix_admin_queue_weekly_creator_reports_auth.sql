/*
  # Fix: Allow service_role/postgres to queue weekly reports

  `admin_queue_weekly_creator_reports` is intended for admin dashboard usage (authenticated admin JWT),
  but local/sql-tool execution has `auth.uid()` = NULL, causing a false "not authorized".

  This patch allows calls when:
  - `auth.role()` is `service_role`, OR
  - `current_user` is `postgres` (migrations / SQL editor),
  while keeping the admin check for regular authenticated users.
*/
