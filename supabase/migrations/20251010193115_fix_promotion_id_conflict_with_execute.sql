/*
  # Fix Promotion ID Conflict Using Dynamic SQL

  ## Overview
  The conflict arises because RETURNS TABLE creates implicit variables with the
  same names as the columns. When we have a column named "promotion_id" in the
  return type, PostgreSQL creates an implicit variable that conflicts with
  column references in INSERT statements.

  ## Solution
  Use EXECUTE for the INSERT statement to avoid the compile-time ambiguity.
  This forces PostgreSQL to resolve column names at runtime when the scope is clear.

  ## Changes
  - Use EXECUTE with dynamic SQL for the problematic INSERT statement
  - All other references remain explicit with table aliases
*/
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Upsert queue state using EXECUTE to avoid ambiguity
      EXECUTE format(
        'INSERT INTO promotion_queue_state (promotion_id, section_key, visibility_score, forced_next_cycle) ' ||
        'VALUES ($1, $2, $3, $4) ' ||
        ...
        'updated_at = now()'
      ) USING v_promotion.id, p_section_key, v_score, v_force;
    END;
  END LOOP;
