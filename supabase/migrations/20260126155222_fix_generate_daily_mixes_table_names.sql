/*
  # Fix Daily Mix Generation Function - Correct Table Names

  1. Updates function to use correct table names:
     - listening_history instead of playback_history
     - duration_listened instead of duration_seconds

  2. Ensures function works with actual database schema
*/
...
BEGIN
  ...
  WITH user_genres AS (
    SELECT
      ...
      COUNT(*) as play_count,
      AVG(lh.duration_listened::float / NULLIF(s.duration, 0)) as avg_completion
  ...
  recommended_songs AS (
    ...
    AND (
      ...
      OR
      -- Has matching moods
      EXISTS (SELECT 1 FROM song_moods sm WHERE sm.song_id = s.id AND sm.mood_id IN (SELECT mood_id FROM user_moods))
      OR
      -- Is trending
      s.play_count > 50
    )
  ...
  distributed_tracks AS (
    SELECT
      ...
      rs.match_genre,
      rs.match_mood,
      ROW_NUMBER() OVER (PARTITION BY (ROW_NUMBER() OVER (ORDER BY rs.recommendation_score DESC) - 1) % v_mixes_per_user ORDER BY rs.recommendation_score DESC) as track_position,
      (ROW_NUMBER() OVER (ORDER BY rs.recommendation_score DESC) - 1) % v_mixes_per_user + 1 as target_mix
  ...
  WHERE dt.track_position <= v_tracks_per_mix
