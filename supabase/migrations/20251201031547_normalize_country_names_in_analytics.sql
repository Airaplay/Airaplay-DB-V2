/*
  # Normalize Country Names in Analytics

  1. Changes
    - Update get_creator_analytics_optimized() to normalize "Nigeria" to "NG" in location data
    - Ensures NG and Nigeria are treated as the same country and merged in analytics

  2. Purpose
    - Fix duplicate entries for NG and Nigeria in Audience Insights
    - Maintain data consistency in analytics reports
*/

-- Drop and recreate the function with country normalization
...
BEGIN
  ...
  -- Get total likes (songs from user_favorites + content from content_likes)
  SELECT
  ...
  WITH ranked_content AS (
    ...
    SELECT
      ...
      END as type,
      play_count,
      metadata->>'thumbnail_url' as cover_url
    FROM public.content_uploads
    WHERE user_id = p_user_id AND status = 'approved'
  )
  SELECT jsonb_agg(
  ...
  -- Normalize country names: "Nigeria" (any case) -> "NG"
  WITH location_data AS (
    SELECT
      CASE 
        WHEN LOWER(COALESCE(lh.detected_country, u.country)) = 'nigeria' THEN 'NG'
        ELSE COALESCE(lh.detected_country, u.country)
      END as country,
      lh.user_id
    ...
    SELECT
      CASE 
        WHEN LOWER(COALESCE(vph.detected_country, u.country)) = 'nigeria' THEN 'NG'
        ELSE COALESCE(vph.detected_country, u.country)
      END as country,
      vph.user_id
...
COMMENT ON FUNCTION public.get_creator_analytics_optimized IS
  'Optimized analytics function with country name normalization: "Nigeria" (any case) is normalized to "NG" to merge duplicate entries in Audience Insights.';
