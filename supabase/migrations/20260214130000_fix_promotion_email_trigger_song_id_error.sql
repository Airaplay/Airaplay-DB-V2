/*
  # Fix promotion email trigger song_id error

  The trigger_send_promotion_active_email function was trying to access NEW.song_id
  and NEW.video_id which don't exist on the promotions table. The table uses
  target_id and promotion_type instead. This migration fixes the function to
...
BEGIN
  ...
  IF NEW.status = 'active' AND (OLD.status IS NULL OR OLD.status != 'active') THEN
    ...
    ELSIF NEW.promotion_type = 'video' AND NEW.target_id IS NOT NULL THEN
      SELECT COALESCE(title, 'Your video')
      INTO v_content_title
      FROM videos WHERE id = NEW.target_id;
...
COMMENT ON FUNCTION trigger_send_promotion_active_email() IS 'Fixed to use target_id and promotion_type instead of non-existent song_id/video_id columns';
