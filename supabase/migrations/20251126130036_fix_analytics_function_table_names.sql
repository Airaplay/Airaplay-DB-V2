/*
  # Fix Analytics Function Table Names

  1. Changes
    - Update get_creator_analytics_optimized() to use correct table names
    - song_likes -> user_favorites (for songs)
    - clip_likes -> content_likes (for videos/clips)
    - Keep content_comments as is (correct)

  2. Purpose
    - Fix "relation does not exist" error in analytics
    - Use actual database table names from schema
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((SELECT COUNT(*) FROM public.content_likes WHERE content_id = ANY(COALESCE(v_content_ids, ARRAY[]::uuid[]))), 0)
  INTO v_total_likes;
...
COMMENT ON FUNCTION public.get_creator_analytics_optimized IS
  'Optimized analytics function that uses database aggregations for fast performance. Returns complete analytics data including play counts, listeners, locations, and growth metrics. Uses correct table names: user_favorites for song likes, content_likes for video/clip likes.';
