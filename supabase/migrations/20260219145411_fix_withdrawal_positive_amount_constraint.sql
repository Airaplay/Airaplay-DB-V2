/*
  # Fix withdrawal transaction amount to satisfy check constraint

  The treat_transactions_amount_bounds constraint requires amount >= 0.
  The trigger uses ABS(NEW.amount) for withdrawal type, so storing a positive
  amount works correctly — the trigger deducts ABS(positive) = positive.

  The previous migration accidentally stored a negative amount to match the
  trigger's ABS() call, but that violates the constraint.

  Fix: Store positive amount. The manual wallet UPDATE was already removed
  in the prior migration, so the trigger is the single deduction path.
*/
...
BEGIN
  ...
  -- Store positive amount (satisfies treat_transactions_amount_bounds check constraint amount >= 0).
  -- The trigger_update_treat_wallet trigger handles the wallet deduction via ABS(NEW.amount).
  -- No manual treat_wallets UPDATE here to prevent double deduction.
