/*
  # Fix Contribution Function Column Name Mismatch
  
  1. Problem
    - `record_listener_contribution()` function tries to insert into `points_earned` column
    - But `listener_contributions` table actually has `contribution_points` column
    - This causes contributions to fail with "column does not exist" error
  
  2. Fix
    - Update the function to use correct column name: `contribution_points`
    - All other logic remains unchanged
  
  3. Impact
    - Users like Di (tfwyoungdi@gmail.com) will now properly earn points when liking songs
    - All contribution activities will work correctly
*/
...
BEGIN
  ...
  -- FIXED: Changed points_earned to contribution_points
