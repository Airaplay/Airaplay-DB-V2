/*
  # Revert Trending Filter - Allow Album Tracks in Trending

  1. Changes
    - Updates `get_shuffled_trending_songs` function to INCLUDE songs that are part of albums
    - Album tracks CAN appear in Trending sections
    - Only New Releases section filters out album tracks
    - This allows popular album tracks to trend naturally
...
-- Update get_shuffled_trending_songs to include album tracks again
...
COMMENT ON FUNCTION public.get_shuffled_trending_songs IS 'Returns trending songs (including album tracks) from the last N days (default 14) with at least 50 play counts, in a shuffled order that changes every 10 minutes.';
