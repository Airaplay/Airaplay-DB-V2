/*
  ...
  1. New Indexes
    - Add composite indexes on playback history tables for faster analytics queries
    - Index on detected_country and detected_country_code fields
    - Index on user_id + created_at for time-based queries
    - Index on content_id for aggregations

  2. Optimized Functions
    - Create get_creator_analytics_optimized() function
    - Uses database aggregations instead of fetching all data
    - Reduces query time from seconds to milliseconds
    - Properly handles detected_country for location analytics

  3. Performance Improvements
    - Reduces data transfer between database and application
    - Uses efficient GROUP BY and aggregate functions
    - Parallel query execution for different metrics
*/

-- Add indexes to listening_history for faster analytics
CREATE INDEX IF NOT EXISTS idx_listening_history_user_created
  ON public.listening_history(user_id, listened_at DESC);

CREATE INDEX IF NOT EXISTS idx_listening_history_song
  ON public.listening_history(song_id);

CREATE INDEX IF NOT EXISTS idx_listening_history_country
  ON public.listening_history(detected_country)
  WHERE detected_country IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_listening_history_validated
  ON public.listening_history(is_validated, listened_at DESC)
  WHERE is_validated = true;

-- Add indexes to video_playback_history for faster analytics
CREATE INDEX IF NOT EXISTS idx_video_playback_user_created
  ON public.video_playback_history(user_id, watched_at DESC);

CREATE INDEX IF NOT EXISTS idx_video_playback_content
  ON public.video_playback_history(content_id);

CREATE INDEX IF NOT EXISTS idx_video_playback_country
  ON public.video_playback_history(detected_country)
  WHERE detected_country IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_video_playback_validated
  ON public.video_playback_history(is_validated, watched_at DESC)
  WHERE is_validated = true;
...
BEGIN
  ...
  -- Get total likes (songs + content)
  SELECT
    COALESCE((SELECT COUNT(*) FROM public.song_likes WHERE song_id = ANY(COALESCE(v_song_ids, ARRAY[]::uuid[]))), 0) +
    COALESCE((SELECT COUNT(*) FROM public.clip_likes WHERE clip_id = ANY(COALESCE(v_content_ids, ARRAY[]::uuid[]))), 0)
  INTO v_total_likes;
...
COMMENT ON FUNCTION public.get_creator_analytics_optimized IS
  'Optimized analytics function that uses database aggregations for fast performance. Returns complete analytics data including play counts, listeners, locations, and growth metrics.';
