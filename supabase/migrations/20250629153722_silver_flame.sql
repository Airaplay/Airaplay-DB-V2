/*
  # Fix corrupted numeric data and update functions to handle edge cases

  1. Data Cleaning
    - Clean up corrupted numeric data in various tables
    - Fix play_count, duration_seconds, and other numeric fields
    - Set default values for NULL or invalid numeric data

  2. Function Updates
    - Drop and recreate functions to handle numeric data safely
    - Add validation to prevent errors from corrupted data
    - Update return types properly

  3. Constraints
    - Add check constraints to prevent future data corruption
    - Ensure numeric fields have appropriate validation
*/
...
SET play_count = 0 
WHERE play_count::text ~ '[^0-9]' OR play_count IS NULL;

-- Clean up songs table - duration_seconds and play_count should be numeric
UPDATE songs 
SET duration_seconds = 0 
WHERE duration_seconds::text ~ '[^0-9]' OR duration_seconds IS NULL;
...
WHERE duration_listened::text ~ '[^0-9]' OR duration_listened IS NULL;

-- Clean up users table - total_earnings should be numeric
UPDATE users 
SET total_earnings = 0.0 
WHERE total_earnings::text ~ '[^0-9.]' OR total_earnings IS NULL;

-- Clean up upload_files table - file_size should be numeric
UPDATE upload_files 
SET file_size = 0 
WHERE file_size IS NOT NULL AND file_size::text ~ '[^0-9]';

-- Drop existing functions before recreating them with updated signatures
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      CASE 
        WHEN cu.play_count IS NOT NULL AND cu.play_count::text ~ '^[0-9]+$' 
        THEN cu.play_count
        ELSE 0
      END, 
      0
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      CASE 
        WHEN s.duration_seconds IS NOT NULL AND s.duration_seconds::text ~ '^[0-9]+$' 
    ...
    COALESCE(
      CASE 
        WHEN s.play_count IS NOT NULL AND s.play_count::text ~ '^[0-9]+$' 
        THEN s.play_count
        ELSE 0
      END, 
      0
