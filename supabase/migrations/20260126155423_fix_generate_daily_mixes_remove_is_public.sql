/*
  # Fix Daily Mix Generation Function - Remove is_public Check

  1. Updates function to remove non-existent is_public column check
  2. Simplifies query to only check for audio_url existence
*/

-- Drop and recreate the function without is_public check
...
BEGIN
  ...
  WITH user_genres AS (
    SELECT
      ...
      COUNT(*) as play_count,
      AVG(lh.duration_listened::float / NULLIF(s.duration_seconds, 0)) as avg_completion
