/*
  # Fix Trending Near You with Smart Fallback Approach
  
  1. Changes
    - Updates `get_trending_near_you_songs` function to use smart fallback
    - Falls back from admin threshold → minimum 1 play if insufficient songs
    - Ensures country-specific sections always display content when available
    
  2. Why Smart Fallback for Country-Specific Trending?
    - Individual countries have less activity than global trending
    - New/smaller markets need content to display
    - Songs with 1-15 plays are still locally "trending"
    - Prevents empty sections while maintaining credibility
    
  3. Fallback Strategy
    - Try admin-defined threshold first (current: 16 plays)
    - If < 10 songs, fallback to minimum 1 play requirement
    - Order by play_count DESC to show most popular first
    - Country-specific filtering maintained throughout
*/

-- Drop existing function
DROP FUNCTION IF EXISTS get_trending_near_you_songs(text, integer, integer);

-- Recreate with smart fallback
...
BEGIN
  ...
  LEFT JOIN playback_history ph ON s.id = ph.song_id 
    AND ph.created_at >= NOW() - (days_param || ' days')::interval
  ...
  HAVING COUNT(ph.id) >= threshold_count
